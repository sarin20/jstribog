...
public class TestB {
    ...
    public static final byte[] MSG_SHORT = new byte[]{
    ...
    public static final int[] MSG_LONG = new int[]{
    ...
    private MessageDigest getDigest() throws NoSuchAlgorithmException, NoSuchProviderException {
        return MessageDigest.getInstance("StribogB512", "JStribog");
    }
    ...
    @Test
    public void testInstall() throws NoSuchAlgorithmException, NoSuchProviderException {
        MessageDigest md = getDigest();
        Assert.assertNotNull(md);
    }
    
    @Test
    public void testJCA512B_Short() throws NoSuchAlgorithmException, NoSuchProviderException {        
        MessageDigest md = getDigest();
        ...
        }), md.digest(reverse(MSG_SHORT)));
    } 
    
    @Test
    public void testJCA512B_Reset() throws NoSuchAlgorithmException, NoSuchProviderException {        
        MessageDigest md = getDigest();
        md.digest(reverse(MSG_SHORT));
        md.reset();
        md.update(reverse(intToByte(MSG_LONG)));
        Assert.assertArrayEquals(intToByte(new int[]{
    ...
    @Test
    public void testJCA512B_Long() throws NoSuchAlgorithmException, NoSuchProviderException {        
        MessageDigest md = getDigest();
        md.update(reverse(intToByte(MSG_LONG)));
        Assert.assertArrayEquals(intToByte(new int[]{
            0x28, 0xFB, 0xC9, 0xBA, 0xDA, 0x03, 0x3B, 0x14, 0x60, 0x64, 0x2B, 0xDC, 0xDD, 0xB9, 0x0C, 0x3F, 0xB3, 0xE5, 0x6C, 0x49, 0x7C, 0xCD, 0x0F, 0x62, 0xB8, 0xA2, 0xAD, 0x49, 0x35, 0xE8, 0x5F, 0x03, 0x76, 0x13, 0x96, 0x6D, 0xE4, 0xEE, 0x00, 0x53, 0x1A, 0xE6, 0x0F, 0x3B, 0x5A, 0x47, 0xF8, 0xDA, 0xE0, 0x69, 0x15, 0xD5, 0xF2, 0xF1, 0x94, 0x99, 0x6F, 0xCA, 0xBF, 0x26, 0x22, 0xE6, 0x88, 0x1E
    ...
    @Test
    public void testJCA256B() throws NoSuchAlgorithmException, NoSuchProviderException {
        MessageDigest md = MessageDigest.getInstance("StribogB256", "JStribog");
        Assert.assertNotNull(md);
        md.update(reverse(MSG_SHORT));
    ...
    private Byte[] reverse(Byte[] ba) {
        Byte[] result = new Byte[ba.length];
        List<Byte> lst = Arrays.asList(ba);
